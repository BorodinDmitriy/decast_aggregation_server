...
class GetPayBill(APIView):
  ...
  def get(self,request,id):
	...
	result["personal_account"] = response.json()["personal_account"]
        ...
        print(report)
	response = requests.post("https://decast-stats.herokuapp.com/pay_bill_report/",data=json.dumps(report))
        #SendGetPayBillReport.delay(json.dumps(report),0)
  ...
  def put(self,request,id):
   ...
   # except:
	...
	response = requests.put(url,data=request.data)
        ...
        stts = "False"
        report["status"] = stts
	response = requests.post("https://decast-stats.herokuapp.com/change_account_report/",data=json.dumps(report))
